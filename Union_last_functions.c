...
This is a local copy by Peter Beck with additions related to the GPR_HB_1D_process.comp
*/
...
int physics_my(enum process choice, double *my,double *k_initial, union data_transfer_union data_transfer, struct focus_data_struct *focus_data, _class_particle *_particle) {
    ...
    switch(choice) {
        ...
        case AF_HB_1D:
            ...
            break;
        #endif
        #ifdef PROCESS_GPR_HB_1D_DETECTOR
        case GPR_HB_1D:
            output = GPR_HB_1D_physics_my(my, k_initial, data_transfer, focus_data, _particle);
            break;
        #endif
...
int physics_scattering(enum process choice, double *k_final, double *k_initial, double *weight, union data_transfer_union data_transfer, struct focus_data_struct *focus_data, _class_particle *_particle) {
    ...
    switch(choice) {
        ...
        #ifdef PROCESS_GPR_HB_1D_DETECTOR
        case GPR_HB_1D:
            output = GPR_HB_1D_physics_scattering(k_final, k_initial, weight, data_transfer, focus_data, _particle);
            break;
        #endif
        #ifdef PROCESS_PHONONSIMPLE_DETECTOR
        case PhononSimple:
